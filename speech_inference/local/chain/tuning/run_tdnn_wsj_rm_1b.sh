...
src_mdl=exp/chain_cleaned_split/tdnn_1d_sp/final.mdl  # Input chain model
...
src_ivec_extractor_dir=exp/nnet3_cleaned_split/extractor  # Source ivector extractor dir used to extract ivector for
...
src_gmm_dir=exp/tri3b # source gmm dir used to generate alignments
                                   # for target data.
gmm_dir=tri3b
src_tree_dir=exp/chain_cleaned_split/tree_sp # chain tree-dir for src data;
...
if [ $stage -le -1 ]; then
  ...
  if ! cmp -s <(grep -v "^#" $src_lang/phones.txt) <(grep -v "^#" data/lang/phones.txt); then
    ...
    cp -r data/lang_chain $lang_src_tgt
  fi
fi
...
local/nnet3/run_ivector_common.sh --stage $stage \
 ...
 if [ $stage -le 4 ]; then
   ...
   nj=$(cat exp/tri3b_ali_train_clean_360_trusted_hires_sp/num_jobs) || exit 1;
   steps/align_fmllr_lats.sh --nj $nj --cmd "$train_cmd" \
     --generate-ali-from-lats true \
     data/$lores_train_data_dir $lang_src_tgt $ali_dir $lat_dir || exit 1;
...
if [ $stage -le 8 ]; then
  ...
  # the lang directory.
  ivec_opt=""
  if $use_ivector;then ivec_opt="--online-ivector-dir exp/nnet3${nnet_affix}/ivectors_test_clean_hires" ; fi
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_test_tgsmall $dir $dir/graph
  for decode_set in test_clean; do (

    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --scoring-opts "--min-lmwt 1" --use_gpu true --num_threads 10 \
      --nj $decode_nj --cmd "$decode_cmd" $ivec_opt \
      $dir/graph data/${decode_set}_hires $dir/decode_${decode_set}_tgsmall || exit 1;
      ) || touch $dir/.error &
